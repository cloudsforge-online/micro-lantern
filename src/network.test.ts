 ...
 *
 * lantern serves BOTH estates from one process since the network consolidation (micro-deploy
...
describe('the handle a request gets', () => {
  ...
  it('REFUSES when this deployment holds no handle for that network', () => {
    ...
    // file a testnet service's error under mainnet, and every layer above would agree it worked —
    // including the alert that then pages somebody about the wrong estate.
...
describe('the operational endpoints are exempt, and only they', () => {
  ...
  it('does not exempt anything that reads or writes', () => {
    for (const p of ['/v1/events', '/v1/issues', '/v1/rum']) {
