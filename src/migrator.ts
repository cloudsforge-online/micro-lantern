...
try {
  const result = await migrate(sql as unknown as Sql, MIGRATIONS, {
    service: SERVICE,
    // See the note on BASELINE_VERSION: zero for a new service, which makes this a no-op. The
    // frozen service's tables carry neither the trace columns nor the status ladder and are not
    // the same tables, so there is nothing to adopt.
