 ...
 * with this service's dependency bag and returns specs the kernel can mount without ever seeing the
 * token, the verifier or the database. That is what lets a second module be mounted beside these
...
import { listOpenIssues } from './issues.ts'
 ...
 *
 * This is the former `handle`: it used to sit between the kernel and `dispatch`, and it wrapped
 * ROUTING as well as the route. Now that a spec is one closure, the wrap is per handler — the miss
 * below included, so a throw while answering an unknown path is mapped exactly as it always was.
 */
function guarded(
  handle: (ctx: RequestContext) => Promise<Reply>,
  deps: ServerDeps,
): (ctx: RequestContext) => Promise<Reply> {
  return async (ctx) => {
    try {
...
/** Map a thrown failure to the reply it deserves. Split out so `guarded` can post-process it. */
...
/**
 * The reply when no route matched — the former `dispatch`'s no-route branch, unchanged.
 *
 * Mounted as a `fallback` spec rather than left to the kernel, because WHICH paths exist is a
 * property of the routes above, not of the plumbing.
 */
function unmatched(deps: ServerDeps): (ctx: RequestContext) => Promise<Reply> {
  return async (ctx) => {
    // ────────────────────────────────────────────────────────────────────────────────────────
...
/**
 * Every route this service serves, each handler already closed over `deps`.
 *
 * The list is otherwise exactly what `buildRoutes` returned: same methods, same paths, same order.
 * Order matters — matching is first-wins.
 */
export function createRoutes(deps: ServerDeps): readonly RouteSpec[] {
  const define = (
    method: string,
    path: string,
    handler: (ctx: RequestContext) => Promise<Reply>,
  ): RouteSpec => ({ method, path, handle: guarded(handler, deps) })
  ...
  return [
     ...
     */
    define('POST', '/otlp/v1/logs', async (ctx) => {
     ...
     */
    define('POST', '/ingest/client', async (ctx) => {
    ...
    define('OPTIONS', '/ingest/client', async (ctx) => {
      const origin = headerOf(ctx.req, 'origin')
     ...
     */
    define('GET', '/v1/requests/:requestId', async (ctx) => {
      await authorise(ctx, deps)
    ...
    define('GET', '/v1/issues', async (ctx) => {
      await authorise(ctx, deps)
    ...
    define('GET', '/v1/events', async (ctx) => {
      await authorise(ctx, deps)
    ...
    // Last, and matched by nothing: the miss. `path` is the label the metrics already used for a
    // request that matched no route, so the series is unchanged.
    { method: '*', path: 'unmatched', fallback: true, handle: guarded(unmatched(deps), deps) },
  ]
}
