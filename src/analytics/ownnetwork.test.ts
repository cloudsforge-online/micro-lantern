 ...
 *
 * **It reads `module.ts`.** Wave M1b folded this service into micro-lantern's process, so its
 * composition root moved from `index.ts` to `createAnalyticsModule` — the same lines, in a function
 * the host calls. The assertions below did not change; only the file they read did. A source
 * assertion left pointing at a file its subject has left passes vacuously, which is worse than not
 * having one, so the last case in each block is the one that fails if this file finds nothing.
 */
